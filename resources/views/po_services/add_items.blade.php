            ...
            <div class="card">
                ...
                <div class="card-body">
                    <dl class="row">
                        ...
                        <dd class="col-sm-8">: IDR
                            {{ $item_services ? number_format($item_services->sum('amount'), 2) : '-' }}</dd>
                        <dt class="col-sm-4">VAT</dt>
                        <dd class="col-sm-8">: IDR
                            {{ $po->is_vat == 1 ? number_format($item_services->sum('amount') * 0.11, 2) : '-' }}</dd>
                        <dt class="col-sm-4">Total Amount</dt>
                        <dd class="col-sm-8">: IDR
                            <b>{{ $po->is_vat == 1 ? number_format($item_services->sum('amount') * 1.11, 2) : number_format($item_services->sum('amount'), 2) }}</b>
                        </dd>
                ...
                <div class="card-header">
                    ...
                    <form action="{{ route('item_service.delete_all', $po->id) }}" method="POST" id="delete-items">@csrf
                        @method('DELETE')
                        <button type="submit" form="delete-items"
                            class="btn btn-sm btn-danger float-right {{ $po->print_count > 2 ? 'disabled' : '' }}"
                            onclick="return confirm('Are You sure You want to DELETE ALL ITEMS in this PO?') "><i
                                class="fas fa-trash"></i> Delete All Items</button>
                    </form>
                </div>
                <div class="card-body">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="item_desc"
                                class="form-control @error('item_desc') is-invalid @enderror">
                            @error('item_desc')
                                <div class="invalid-feedback">
                                    {{ $message }}
                                </div>
