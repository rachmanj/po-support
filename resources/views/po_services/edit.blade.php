                    ...
                    <div class="card-body">
                            ...
                            <select name="project_code" id="project_code"
                                ...
                                @foreach ($projects as $project)
                                    <option value="{{ $project }}"
                                        {{ $po->project_code === $project ? 'selected' : '' }}>{{ $project }}</option>
                                @endforeach
                            </select>
                            @error('project_code')
                                <div class="invalid-feedback">
                            ...
                            <select name="vendor_code" id="vendor_code"
                                ...
                                @foreach ($vendors as $vendor)
                                    <option value="{{ $vendor->code }}"
                                        {{ $po->vendor_code === $vendor->code ? 'selected' : '' }}>{{ $vendor->name }}
                                        - {{ $vendor->code }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <div class="form-check">
                                <input name="is_vat" value="1" class="form-check-input" type="radio"
                                    {{ $po->is_vat == true ? 'checked' : '' }} id="yes">
                                <label for="yes" class="form-check-label">Yes</label>
                            </div>
                            <div class="form-check">
                                <input name="is_vat" value="0" class="form-check-input" type="radio" id="no"
                                    {{ $po->is_vat == false ? 'checked' : '' }}>
